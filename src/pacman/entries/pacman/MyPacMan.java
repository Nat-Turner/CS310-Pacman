...
if(game.getActivePowerPillsIndices().length>0) {
    int PowerPill=game.getClosestNodeIndexFromNodeIndex(game.getPacmanCurrentNodeIndex(),game.getActivePowerPillsIndices(),PATH);
   ...
   // System.out.println(game.getShortestPathDistance(game.getPacmanCurrentNodeIndex(),ghostLocation));
    ...
    if(game.getShortestPathDistance(game.getPacmanCurrentNodeIndex(),PowerPill)==1&&areAllGhostsOut(game)==false){
    ...
    int closest = game.getClosestNodeIndexFromNodeIndex(game.getPacmanCurrentNodeIndex(), game.getActivePillsIndices(), PATH);
    myMove = game.getNextMoveTowardsTarget(game.getPacmanCurrentNodeIndex(), closest, PATH);
        ...
        if(game.isGhostEdible(closestGhost)==true){
            myMove= game.getNextMoveTowardsTarget(game.getPacmanCurrentNodeIndex(),game.getGhostCurrentNodeIndex(closestGhost),PATH);
        }
	...
	public Constants.GHOST findClosestGhost(Game g){
		...
		Constants.GHOST list[]={Constants.GHOST.INKY,Constants.GHOST.PINKY,Constants.GHOST.SUE,Constants.GHOST.BLINKY};
		...
		int shortest =g.getShortestPathDistance(g.getPacmanCurrentNodeIndex(),g.getGhostCurrentNodeIndex(Constants.GHOST.INKY));
		int index=0,best =0;
		for(int i: Ghosts){
